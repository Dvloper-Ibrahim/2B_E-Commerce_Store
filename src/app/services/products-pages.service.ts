...
export class ProductsPagesService {
  ...
  getAllProduct(): Observable<IProduct[]> {
    return this.httpClient.get<IProduct[]>(
      `${environment.BaseApiUrl}/api/product`
    );
  }

  getproductsById(prodId: number): Observable<IProduct> {
    return this.httpClient.get<IProduct>(
      `${environment.BaseApiUrl}/api/product/${prodId}`
    );
  }

  getproductsByName(prodName: string): Observable<IProductsPages> {
    return this.httpClient.get<IProductsPages>(
      `${environment.BaseApiUrl}/productsPages/${prodName}`
    );
  }

  getProductInformationTable(): Observable<InformatinTableProducts[]> {
    return this.httpClient.get<InformatinTableProducts[]>(
      `${environment.BaseApiUrl}/productsPages`
    );
  }

  getProductsByCategoryId(categoryId: number): Observable<IProduct[]> {
    return this.httpClient.get<IProduct[]>(
      `${environment.BaseApiUrl}/api/product/category/${categoryId}`
    );
  }

  getProductsBySubCategoryId(subCategoryId: number): Observable<IProduct[]> {
    return this.httpClient.get<IProduct[]>(
      `${environment.BaseApiUrl}/api/Product/parentSubCategory/${subCategoryId}`
    );
  }
  ...
  getProductsBySubSubCategoryId(
    ...
    return this.httpClient.get<IProduct[]>(
      `${environment.BaseApiUrl}/api/Product/childSubCategory/${subSubCategoryId}`
    );
  }
  ...
  searchFor(str: string): Observable<IProduct[]> | undefined {
    if (!!str) {
      return this.httpClient.get<IProduct[]>(
        `${environment.BaseApiUrl}/api/Product/search?query=${str}`
      );
    }
    return undefined;
  }
